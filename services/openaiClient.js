...
const client = new OpenAI({ apiKey: OPENAI_API_KEY, timeout: Number(OPENAI_TIMEOUT_MS || 10000) });
...
${(contextChunks || []).map((c, i) => `(${i+1}) ${c.text} [Fonte: ${c.source}${c.date ? ", " + c.date : ""}]`).join("\n")}`.trim();

  const messages = [{ role: "system", content: sys }];
  for (const m of (history || [])) messages.push({ role: m.role, content: m.content });
  messages.push({ role: "user", content: user });
  const completion = await client.chat.completions.create({ model, messages, temperature: 0.3 });
